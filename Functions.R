...
chfun_heston <- function(S0, V0, alpha, lambda, sigmaV, r, rho, Tt, w){
    ...
    # w: point at which to evaluate the function
    # Output:
    # Characteristic function of log (St) in the Heston mode
    ...
    y <- exp(C*(alpha/lambda) + D*V0 + i*w*log(S0*exp(r*Tt)))
}
